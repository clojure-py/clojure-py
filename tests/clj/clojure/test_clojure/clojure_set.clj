...
;;   * `IllegalArgumentException` for 0-arg `(set/intersection)` →
;;     `clojure._core/ArityException`. Our runtime raises ArityException
;;     when no matching arity is found; vanilla's `intersection` declares
;;     no 0-arity, so a zero-arg call fails the same way under a
...
(deftest test-intersection
  ...
  (is (thrown? clojure._core/ArityException (set/intersection)))

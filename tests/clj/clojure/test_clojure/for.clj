...
;;   * `(throw (Exception. "..."))` → our runtime has no Java Exception
;;     constructor; use `clojure._core/IllegalStateException`.
;;   * `(Integer. s)` string-to-int → `(builtins/int s)`.
;;   * The last clause of `While-and-When-Same-Binding` uses `Math/abs` and
;;     the ratio literal `1/3` — neither is implemented yet. Dropped.
...
(defn only
  ...
  [n]
  (lazy-cat (range n)
            (throw (clojure._core/IllegalStateException
                     "consumer went too far in lazy seq"))))
...
(deftest-both While-and-When-Same-Binding
  ...
  ;; Dropped vanilla's last clause — uses `Math/abs` and the ratio literal
  ;; `1/3`. Neither is implemented in this runtime yet.
  )

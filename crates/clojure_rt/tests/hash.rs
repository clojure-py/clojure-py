//! End-to-end tests for `IHash` per-primitive impls. The numeric
...
fn h(v: Value) -> i64 {
    rt::hash(v).as_int().expect("IHash returned non-int Value")
}
...
#[test]
fn hash_float_negative_zero_is_zero() {
    init();
...
#[test]
fn hash_float_one_matches_java_double_hashcode() {
    init();
...
#[test]
fn hash_int_and_float_with_same_numeric_value_differ() {
    init();

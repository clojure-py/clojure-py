...
impl SymbolObj {
    ...
    unsafe fn replace_meta(this: Value, new_meta: Value) -> Value {
        ...
        Self::alloc(body.ns, body.name, new_meta, AtomicI32::new(0))
    }
}
...
clojure_rt_macros::implements! {
    impl IHash for SymbolObj {
        fn hash(this: Value) -> Value {
            let body = unsafe { SymbolObj::body(this) };
            let cached = body.hash.load(Ordering::Relaxed);
            if cached != 0 {
                return Value::int(cached as i64);
            }
            let name_str = unsafe { StringObj::as_str_unchecked(body.name) };
            let name_h = murmur3::hash_unencoded_chars(name_str);
            let ns_h = if body.ns.is_nil() {
                0
            } else {
                let s = unsafe { StringObj::as_str_unchecked(body.ns) };
...
clojure_rt_macros::implements! {
    impl IEquiv for SymbolObj {
        fn equiv(this: Value, other: Value) -> Value {
            ...
            let a = unsafe { SymbolObj::body(this) };
            let b = unsafe { SymbolObj::body(other) };

            // Name comparison: both are StringObj. Compare bytes directly.
            let a_name = unsafe { StringObj::as_str_unchecked(a.name) };
            let b_name = unsafe { StringObj::as_str_unchecked(b.name) };
            if a_name != b_name {
                return Value::FALSE;
            }
            ...
            // or differ → not equal.
            if a.ns.is_nil() && b.ns.is_nil() {
                return Value::TRUE;
            }
            if a.ns.is_nil() || b.ns.is_nil() {
                return Value::FALSE;
            }
            let a_ns_s = unsafe { StringObj::as_str_unchecked(a.ns) };
            let b_ns_s = unsafe { StringObj::as_str_unchecked(b.ns) };
            if a_ns_s == b_ns_s { Value::TRUE } else { Value::FALSE }
        }
    }
}
...
clojure_rt_macros::implements! {
    impl INamed for SymbolObj {
        fn namespace(this: Value) -> Value {
            let v = unsafe { SymbolObj::body(this) }.ns;
            crate::rc::dup(v);
            v
        }
        fn name(this: Value) -> Value {
            let v = unsafe { SymbolObj::body(this) }.name;
            crate::rc::dup(v);
            v
        }
    }
...
clojure_rt_macros::implements! {
    impl IMeta for SymbolObj {
        fn meta(this: Value) -> Value {
            let v = unsafe { SymbolObj::body(this) }.meta;
            crate::rc::dup(v);
            v
        }
    }

...
//! `TYPE_FLOAT64`, `TYPE_CHAR`) is registered in the `type_registry`
//! at `init()` time and given a `TYPE_ID` cell of the same shape as
//! `register_type!`-generated cells. This lets `implements!` install
//! a primitive-targeted impl through the same per-type dispatch
//! machinery as heap types — no fallback, no tag-case-analysis layer.
//!
//! Python objects used to live here as a primitive `TYPE_PYOBJECT`
//! tag with borrow semantics; that's been replaced by heap-allocated
//! `pyowned`-wrapped Values that flow through the standard heap path
//! with proper refcount discipline. See `clojure_py::pyowned`.
//!
//! Usage:
//!
//! ```ignore
...
use crate::value::{
    TypeId,
    TYPE_BOOL, TYPE_CHAR, TYPE_FLOAT64, TYPE_INT64, TYPE_NIL,
};

...
impl Value {
    ...
    /// Build a heap-pointing Value.
    ///
    /// # Safety
    /// `h` must be non-null, 16-aligned, and point at a live `Header` whose
    /// `type_id` is `>= FIRST_HEAP_TYPE`. Debug builds assert these.
    #[inline(always)]
    pub unsafe fn from_heap(h: *const crate::header::Header) -> Value {

...
use crate::compiler::capture_analysis::{body_has_fn_capturing, count_outer_refs_in_forms};
use crate::compiler::peephole::fuse_deref_invoke_pass;
...
pub enum Resolved {
    /// Local slot holding a plain value. Emit `LoadLocal(slot)`.
    LocalVar(u16),
    /// Local slot holding a `LetfnCell`. Emit `LoadLocal(slot) + LetfnCellGet`.
    LocalCell(u16),
    /// Captured value at `capture_ix`. Emit `LoadCapture(ix)`.
    CaptureVar(u16),
    /// Captured `LetfnCell` at `capture_ix`. Emit `LoadCapture(ix) + LetfnCellGet`.
    CaptureCell(u16),
    Var(u16),
...
type SpecialFormFn = for<'py> fn(&mut Compiler, Python<'py>, PyObject) -> PyResult<()>;

/// Registry of unqualified symbols whose list forms are compiled directly
/// instead of going through `compile_invoke`. Order doesn't matter;
/// `compile_list_form` walks this table linearly.
const SPECIAL_FORMS: &[(&str, SpecialFormFn)] = &[
    ("quote", Compiler::compile_quote),
    ("if", Compiler::compile_if),
    ("do", Compiler::compile_do),
    ("let*", Compiler::compile_let_form),
    ("loop*", Compiler::compile_loop_form),
    ("letfn*", Compiler::compile_letfn),
    ("recur", Compiler::compile_recur),
    ("var", Compiler::compile_var_form),
    ("fn*", Compiler::compile_fn_form),
    ("def", Compiler::compile_def),
    ("set!", Compiler::compile_set_bang),
    ("throw", Compiler::compile_throw),
    ("try", Compiler::compile_try),
    (".", Compiler::compile_dot_legacy),
];
...
impl Compiler {
    ...
    fn resolve_local_or_capture(&mut self, name: &str) -> Option<Resolved> {
        ...
        {
            return Some(if lb.is_letfn_cell {
                Resolved::LocalCell(lb.slot)
            } else {
                Resolved::LocalVar(lb.slot)
            });
        }
        ...
        match source {
            CaptureSource::Capture(ix) => Some(if is_letfn_cell {
                Resolved::CaptureCell(ix)
            } else {
                Resolved::CaptureVar(ix)
            }),
    ...
    pub fn compile_form(&mut self, py: Python<'_>, form: PyObject) -> PyResult<()> {
        ...
        if let Ok(sym_ref) = b.cast::<Symbol>() {
            match self.resolve_symbol(py, sym_ref.get())? {
                Resolved::LocalVar(slot) => { self.emit_load_local(slot); }
                Resolved::LocalCell(slot) => {
                    self.emit_load_local(slot);
                    self.emit(Op::LetfnCellGet);
                }
                Resolved::CaptureVar(ix) => { self.emit(Op::LoadCapture(ix)); }
                Resolved::CaptureCell(ix) => {
    ...
    fn compile_list_form(
        ...
        if let Ok(sym_ref) = hb.cast::<Symbol>() {
            ...
            if s.ns.is_none() {
                let n = s.name.as_ref();
                if let Some(&(_, handler)) = SPECIAL_FORMS.iter().find(|(k, _)| *k == n) {
                    return handler(self, py, list_py);
                }
                // .-attr sugar → GetAttr
    ...
    fn compile_let_form(&mut self, py: Python<'_>, list_py: PyObject) -> PyResult<()> {
        self.compile_let(py, list_py, false)
    }

    fn compile_loop_form(&mut self, py: Python<'_>, list_py: PyObject) -> PyResult<()> {
        self.compile_let(py, list_py, true)
    }

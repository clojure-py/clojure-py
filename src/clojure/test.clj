...
;;
;; Port of clojure/test.clj. See the original docstring (preserved on
;; the ns form below) for usage. Adaptations from JVM:
;;
;;   - clojure.stacktrace/print-cause-trace replaced with Python's
;;     traceback.print_exception. The JVM stacktrace-filtering logic
;;     in do-report (which strips frames whose class names start with
;;     "java.lang." / "clojure.test$" / etc.) is skipped — Python
;;     tracebacks aren't class-name based.
;;   - clojure.string/starts-with? not yet ported; we use the
;;     existing .startsWith fallback on Python str.
;;   - Thread.currentThread / .getStackTrace for finding the current
;;     test's file+line is replaced with a sentinel — file/line is
;;     left nil for fail reports. Pytest integration will surface
;;     better location info via the test fn's metadata.

(ns
  ^{:author "Stuart Sierra, with contributions and suggestions by
  Chas Emerick, Allen Rohner, and Stuart Halloway",
    :doc "A unit testing framework. See the JVM Clojure docs for full
  usage; the macros (is, are, testing, deftest, deftest-, set-test,
  with-test, use-fixtures) and runners (test-var, test-vars,
  test-all-vars, test-ns, run-tests, run-all-tests, run-test,
  successful?) work as in JVM Clojure with the adaptations called
  out at the top of test.clj."}
  clojure.test
...
(defn do-report
  "Add file and line information to a test result and call report.
   ...
   Adaptation: JVM filters Throwable.getStackTrace by class name to
   pick the user-frame of an assertion — Python tracebacks aren't
   class-name shaped, so we don't try. The pytest plugin reads file
   and line info off the test fn's var metadata instead."
  {:added "1.2"}
  [m]
  (report m))
  ...
  (with-test-out
    ...
    (let [actual (:actual m)]
      (if (instance? Throwable actual)
        ;; Adaptation: clojure.stacktrace/print-cause-trace →
        ;; Python's traceback.print_exception. Honor *stack-trace-depth*
        ;; via the `limit` arg.
        (py.traceback/print_exception actual)
        (prn actual)))))
...
(defmacro deftest
  ...
  Adaptation: JVM evaluates metadata expressions at def time, so
  `^{:test (fn [] ...)}` resolves to a real fn before the var binds.
  Our compiler doesn't (yet) eval metadata exprs, so we attach the
  :test fn separately via alter-meta! after defining the var."
  {:added "1.1"}
  [name & body]
  (when *load-tests*
    `(do
...
(defn test-vars
  ...
  {:added "1.6"}
  [vars]
  (doseq [pair (group-by (comp :ns meta) vars)]
    (let [ns (key pair)
          vars (val pair)
          once-fixture-fn (join-fixtures (::once-fixtures (meta ns)))

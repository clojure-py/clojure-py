...
(defn sort-by
  ...
  ([keyfn ^java.util.Comparator comp coll]
   ;; JVM calls `(.compare comp ...)`; in clojure-py our `comp` argument
   ;; is just a callable (no Comparator class), so invoke it directly.
   ;; The Arrays/sort shim takes care of bool-vs-int return semantics.
                     ...
                     (let [gmap (gensym "map__")
                       ...
                       (loop [ret (-> bvec (conj gmap) (conj v)
                                      (conj gmap) (conj `(if (seq? ~gmap)
                                                           (if (next ~gmapseq)
                                                             ;; JVM: PersistentArrayMap/createAsIfByAssoc — snake_case in clojure-py.
                                                             (clojure.lang.PersistentArrayMap/create_as_if_by_assoc (to-array ~gmapseq))
                                                             ;; JVM: PersistentArrayMap/EMPTY — Cython cdef classes don't allow
                                                             ;; class-attribute assignment, so the singleton is exposed at the
                                                             ;; module level instead.
                                                             (if (seq ~gmapseq) (first ~gmapseq) clojure.lang.PERSISTENT_ARRAY_MAP_EMPTY))
                                                           ~gmap))
                                      ((fn [ret]
                                         (if (:as b)
                                           (conj ret (:as b) gmap)
...
;; JVM source: (import '(java.util.concurrent BlockingQueue
;;                                              LinkedBlockingQueue)).
;; Use def aliases like elsewhere — clojure.lang has both shims.
(def BlockingQueue       clojure.lang.BlockingQueue)
(def LinkedBlockingQueue clojure.lang.LinkedBlockingQueue)
